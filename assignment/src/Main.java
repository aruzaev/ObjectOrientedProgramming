...
class TaxCalculator {
    ...
    public double calculateFederalTax(double salary) {
        ...
        if (salary <= FEDERAL_BRACKET1) {
            ...
            tax = FEDERAL_BRACKET1 * FEDERAL_RATE1 +
                    (FEDERAL_BRACKET2 - FEDERAL_BRACKET1) * FEDERAL_RATE2 + // if inside of a bracket, take the outputs of the other brackets on top of current brackeet
    ...
    public double calculateOntarioTax(double salary) {
        ...
        } else if (salary <= ONTARIO_BRACKET2) {
            tax = ONTARIO_BRACKET1 * ONTARIO_RATE1 +
                    (salary - ONTARIO_BRACKET1) * ONTARIO_RATE2;
        } else if (salary <= ONTARIO_BRACKET3) {
            tax = ONTARIO_BRACKET1 * ONTARIO_RATE1 +
                    (ONTARIO_BRACKET2 - ONTARIO_BRACKET1) * ONTARIO_RATE2 +
                    (salary - ONTARIO_BRACKET2) * ONTARIO_RATE3;
        } else if (salary <= ONTARIO_BRACKET4) {
            tax = ONTARIO_BRACKET1 * ONTARIO_RATE1 +
                    (ONTARIO_BRACKET2 - ONTARIO_BRACKET1) * ONTARIO_RATE2 +
                    (ONTARIO_BRACKET3 - ONTARIO_BRACKET2) * ONTARIO_RATE3 +
                    (salary - ONTARIO_BRACKET3) * ONTARIO_RATE4;
        } else {
            tax = ONTARIO_BRACKET1 * ONTARIO_RATE1 +
                    (ONTARIO_BRACKET2 - ONTARIO_BRACKET1) * ONTARIO_RATE2 +
                    (ONTARIO_BRACKET3 - ONTARIO_BRACKET2) * ONTARIO_RATE3 +
                    (ONTARIO_BRACKET4 - ONTARIO_BRACKET3) * ONTARIO_RATE4 +
                    (salary - ONTARIO_BRACKET4) * ONTARIO_RATE5;
        }
        return tax;
    }
...
public class Main {
    public static void main(String[] args) {
        ...
        // creating a new TaxCalculator object
        TaxCalculator calculator = new TaxCalculator();

        // storing in local variables and calling methods
        ...
        // using c style printf to format the numbers to two decimals
